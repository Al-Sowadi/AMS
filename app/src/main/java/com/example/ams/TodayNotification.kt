...
class TodayNotification : Fragment() {
    ...
    override fun onCreateView(
        ...
        val s:String = currentDate.toString()
        var ss:String =""
        for(i in s ){
            if(!i.equals('-')){
                ss=ss+ i
        ...
        val viewToday =  inflater.inflate(R.layout.fragment_today_notification, container, false)
        val databaseToday = FirebaseDatabase.getInstance()
        val myRefToday = databaseToday.getReference("AllData/Student")
...
//            println(formatter.format(date))
        ...
        val recyclerviewToday = viewToday.findViewById<RecyclerView>(R.id.recyclerviewTodat)
        recyclerviewToday?.layoutManager = LinearLayoutManager(viewToday.context)
...
//        myRefToday.addListenerForSingleValueEvent(getdataToday)
